...
%
%**********swm note***************
%In order to specify the axes into which the arrow is drawn, I added a
%global variable (since I didn't want to change anything substantial.)
%Thus if 
%global haxis111
%is defined, the arrow will be drawn in axis haxis111.  Otherwise, it will
%be drawn in current axis.  It was found that simply using axes(haxis111)
%before the call was sometimes not sufficient for accomplishing this.
%
%   EXAMPLES:
%
%     % 2D vectors
...
if isempty(haxis111)
    ...
    if ishandle(haxis111)
        ax=haxis111;
        fig=get(haxis111,'parent');
    else
        fig=gcf; ax=gca;
    end
end    
  ...
  if length(lw)==1
    if lw>0
      if OneColor && ls(end)~='*' && n>1 % single color, linestyle/width
        ...
        H1=plot3(haxis111,P(:,1),P(:,2),P(:,3),'LineWidth',lw);
      else                               % single linewidth
        H1=plot3(haxis111,[p1(:,1),p2(:,1)]',[p1(:,2),p2(:,2)]',...
...
if xys 
    ...
    yr=get(ax,'ylim');
    %zr=get(ax,'zlim');
    %Changed by swm in Aug 2016.  It was found that if only two dimensions
    %are used to define p1 and p2, then zr turns out to be [-1 1], which
    %can be much larger than the other dimensions.
    if c1==3
        zr=get(ax,'zlim');
    else
        zr=[0 0];
...
G=surface(x/2,y/2,z,'parent',haxis111); dar=diag(ar);
